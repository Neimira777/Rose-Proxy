...
- The real Neimira logo (three interlocking rings) exists as inline SVG
  in `public/signup.html` (the `<svg>` in the page header) — that's the
  source of truth, not a separate image file. A rendered PNG of it lives
  at `public/assets/neimira-logo.png` (generated by screenshotting that
  SVG via headless Chromium — LibreOffice/soffice-based rendering is
  broken in at least some Claude Code sandboxes, so Playwright/Chromium
  is the reliable path if it needs re-rendering, e.g. at a different
  size) — use that file directly for letterheads, external documents,
  or anywhere else a raster logo is needed, rather than re-deriving it.
